...
def caculate(comp_tech, recre_act, lgr_penalty, lgr_coeffient):
    min_df = 2 
    ...
    text_clf = Pipeline([('vect', CountVectorizer(min_df = min_df, stop_words = stop_words)),
                         ...
                         ('lgr', LogisticRegression(penalty = lgr_penalty, C = lgr_coeffient))])
...
def get_result(lgr_penalty, lgr_coeffient):
    min_df = 2 
    twenty_train = e.get_data()
    comp_tech, recre_act = e.combine_two_class(twenty_train)
    test_target, predicted, value, mean = caculate(comp_tech, recre_act, lgr_penalty, lgr_coeffient)
...
if __name__ == "__main__":
    lgr_penalty = ['l1', 'l2']
    lgr_coefficient = [0.01, 0.1, 1, 10, 100, 1000]
    for penalty in lgr_penalty:
        for coefficient in lgr_coefficient:
            print penalty, coefficient
            get_result(penalty, coefficient) 
